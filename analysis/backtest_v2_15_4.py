...
def raw_center(row: dict, state: PoolState) -> tuple[float | None, str]:
    ...
    hint = mean_tail(org_records, 10) or mean_tail(svc_records, 10) or mean_tail(all_records, 20)
    candidates = {
    ...
    if route == "bayes_shrink":
        ...
        selected = org_svc_records or org_records or svc_records or all_records
...
def prepare_data(drop_exact_duplicates: bool = False) -> tuple[pd.DataFrame, dict]:
    raw = pd.read_excel(SOURCE, sheet_name="통합낙찰이력")
    source_rows_before_dedup = len(raw)
    if drop_exact_duplicates:
        raw = raw.drop_duplicates().copy()
    quality: dict[str, object] = {
        "source_rows": int(source_rows_before_dedup),
        "rows_after_optional_exact_dedup": int(len(raw)),
        "exact_dedup_applied": bool(drop_exact_duplicates),
        "source_columns": int(len(raw.columns)),
        "columns": [str(column) for column in raw.columns],
        "nulls": {column: int(raw[column].isna().sum()) for column in raw.columns},
        "exact_duplicate_rows": int(raw.duplicated().sum()),
        "duplicate_notice_rows": int(raw.duplicated(subset=["공고번호"], keep=False).sum()),
        "duplicate_notice_keys": int(raw.loc[raw.duplicated(subset=["공고번호"], keep=False), "공고번호"].nunique()),
    }
    data = raw.copy()
    data["_date"] = parse_dates(data["개찰일"])
    data["_value"] = pd.to_numeric(data[RATE], errors="coerce")
    data["_base"] = pd.to_numeric(data["기초금액"], errors="coerce")
    data["_companies"] = pd.to_numeric(data["업체수"], errors="coerce")
    valid = (
        data["_date"].notna()
        & data["_value"].notna()
        & (data["_value"].abs() < 10)
    ...
    data = data.sort_values(["_date", "공고번호", "번호"], kind="mergesort").reset_index(drop=True)
    data["_svc"] = [
        classify_service(name, industry)
        for name, industry in zip(data["공고명"], data["업종"])
    ]
    data["_amt"] = data["_base"].apply(amount_bucket)
    data["_comp"] = data["_companies"].apply(company_bucket)
    data["_region"] = data["지역"].apply(simple_region)
    quality.update(
        {
            "valid_rows": int(len(data)),
            "invalid_or_excluded_rows": int(len(raw) - len(data)),
            "date_min": data["_date"].min().strftime("%Y-%m-%d"),
            "date_max": data["_date"].max().strftime("%Y-%m-%d"),
            "invalid_date_rows": int(data["_date"].isna().sum()),
            "invalid_target_rows_source": int(
                pd.to_numeric(raw[RATE], errors="coerce").isna().sum()
                + (pd.to_numeric(raw[RATE], errors="coerce").abs() >= 10).sum()
            ),
            "missing_base_valid_rows": int(data["_base"].isna().sum()),
            "nonpositive_base_valid_rows": int((data["_base"].fillna(0) <= 0).sum()),
            "missing_company_count_valid_rows": int(data["_companies"].isna().sum()),
            "missing_region_valid_rows": int(data["지역"].isna().sum()),
            "service_counts": data["_svc"].value_counts().to_dict(),
        }
    )
    return data, quality


def metric_row(group: pd.DataFrame, variant: str, label: str, dimension: str) -> dict:
    errors = group[f"{variant}_error"].dropna()
    signed = group[f"{variant}_signed"].dropna()
    return {
        "구분축": dimension,
        "구분": label,
        "모델": variant,
...
def summaries(detail: pd.DataFrame, max_date: pd.Timestamp) -> tuple[pd.DataFrame, pd.DataFrame, pd.DataFrame]:
    periods = {
        "전체": detail,
    ...
    for label, frame in periods.items():
        for variant in ("v2.15.3_current", "v2.15.4_candidate"):
            period_rows.append(metric_row(frame, variant, label, "기간"))

    segment_rows = []
    recent_year = periods["최근 1년"]
    for segment, frame in recent_year.groupby("분석구분", dropna=False):
        for variant in ("v2.15.3_current", "v2.15.4_candidate"):
            segment_rows.append(metric_row(frame, variant, str(segment), "최근1년 구분"))
    for service, frame in recent_year.groupby("용역분류", dropna=False):
        for variant in ("v2.15.3_current", "v2.15.4_candidate"):
            segment_rows.append(metric_row(frame, variant, str(service), "최근1년 분야"))

    daily = (
        detail.groupby("개찰일", as_index=False)
        .agg(
            표본수=("공고번호", "size"),
            현행_MAE=("v2.15.3_current_error", "mean"),
            후보_MAE=("v2.15.4_candidate_error", "mean"),
            현행_030=("v2.15.3_current_error", lambda values_: float((values_ <= 0.30).mean())),
            후보_030=("v2.15.4_candidate_error", lambda values_: float((values_ <= 0.30).mean())),
        )
    )
    for column in ("현행_MAE", "후보_MAE", "현행_030", "후보_030"):
        daily[column] = daily[column].round(4)
    return pd.DataFrame(period_rows), pd.DataFrame(segment_rows), daily
...
def main() -> None:
    ...
    data, quality = prepare_data(drop_exact_duplicates=args.dedup)
    states = {
        "global": PoolState(),
        "electric": PoolState(),
        "kepco_local": PoolState(),
        "kepco_national": PoolState(),
    }
    results: list[dict] = []
    processed = 0
    for date, group in data.groupby("_date", sort=True):
        if len(states["global"].records) >= MIN_PRIOR_ROWS:
            for _, source_row in group.iterrows():
                row = {
                    ...
                    "companies": (
                        float(source_row["_companies"]) if pd.notna(source_row["_companies"]) else None
                    ),
                    "svc": source_row["_svc"],
                    "amt": source_row["_amt"],
                ...
                state = select_state(row, states)
                current, current_model = recommendations(row, state, "v2.15.3_current")
                candidate, candidate_model = recommendations(row, state, "v2.15.4_candidate")
                if not current or not candidate:
                    continue

                def evaluate(recs: list[float]) -> tuple[float, float, float]:
                    signed_values = [rate - row["value"] for rate in recs]
                    best_index = int(np.argmin(np.abs(signed_values)))
                    signed = float(signed_values[best_index])
                    return abs(signed), signed, float(recs[best_index])

                current_error, current_signed, current_best = evaluate(current)
                candidate_error, candidate_signed, candidate_best = evaluate(candidate)
                scope, count = current_scope(row)
                result = {
                    "개찰일": date,
                    "공고번호": row["notice"],
                    "공고명": row["name"],
                    "발주기관": row["org"],
                    "용역분류": SERVICE_LABELS[row["svc"]],
                    "분석구분": segment_name(row),
                    "입찰범위": scope,
                    "참여업체수": count,
                    "기초금액": row["base"],
                    "실제사정율": row["value"],
                    ...
                    "현행추천3": current[2] if len(current) >= 3 else None,
                    "후보추천1": candidate[0] if len(candidate) >= 1 else None,
                    "후보추천2": candidate[1] if len(candidate) >= 2 else None,
                    "후보추천3": candidate[2] if len(candidate) >= 3 else None,
                    "현행최근접추천": current_best,
                    "후보최근접추천": candidate_best,
                    "v2.15.3_current_error": current_error,
                    "v2.15.3_current_signed": current_signed,
                    "v2.15.4_candidate_error": candidate_error,
                    "v2.15.4_candidate_signed": candidate_signed,
                    "현행모델": current_model,
                    "후보모델": candidate_model,
                    "현행추천기준금액1": int(row["base"] * (100 + current[0]) / 100),
                    "후보추천기준금액1": int(row["base"] * (100 + candidate[0]) / 100),
                    "예정가격": source_row.get("예정가격"),
                    "1순위투찰금액": source_row.get("1순위투찰금액"),
                }
        ...
        for _, source_row in group.iterrows():
            ...
            add_to_states(record, states)
        processed += len(group)
        if processed % 5000 < len(group):
            print(f"processed={processed:,}/{len(data):,}", flush=True)

    detail = pd.DataFrame(results)
    max_date = data["_date"].max()
    period, segment, daily = summaries(detail, max_date)

    detail.to_csv(out_dir / "backtest_detail.csv", index=False, encoding="utf-8-sig")
    period.to_csv(out_dir / "period_summary.csv", index=False, encoding="utf-8-sig")
    segment.to_csv(out_dir / "segment_summary.csv", index=False, encoding="utf-8-sig")
    daily.to_csv(out_dir / "daily_summary.csv", index=False, encoding="utf-8-sig")

    quality["backtest_rows"] = int(len(detail))
    quality["burn_in_prior_rows"] = MIN_PRIOR_ROWS
    quality["same_day_leakage_prevented"] = True
    quality["runtime_seconds"] = round(time.time() - started, 2)
    quality["backtest_date_min"] = detail["개찰일"].min().strftime("%Y-%m-%d")
    quality["backtest_date_max"] = detail["개찰일"].max().strftime("%Y-%m-%d")
    ...
    print("\nRECENT ONE YEAR SEGMENTS")
    print(
        segment[
            (segment["구분축"] == "최근1년 구분")
            | ((segment["구분축"] == "최근1년 분야") & segment["구분"].isin(["전기공사", "감리"]))
