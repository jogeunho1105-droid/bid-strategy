# ╔══════════════════════════════════════════════════════════════════╗
# ║     투찰전략 분석 시스템 v2.0 — 구글 드라이브 연동             ║
# ║  드라이브 구조:                                                  ║
# ║  📁 입찰AI분석 (1WgIJEjncgGZXz2SGQ_IhqjGSD15fz0lm)           ║
# ║    ├── 📁 낙찰이력데이터 (1IpNRcYMc7medssJWgLhOkXrKwKOn73H3) ║
# ║    │     └── 발주처별_패턴통계.json                             ║
# ║    ├── 📥 입찰서류함 (1ZDyeqNXKWiJ3lQS9VcoCzeyGxsAOEsoG)     ║
# ║    └── 📊 투찰전략결과 (1vXkPHr2walQJZqUrZ4tZ4FkwmQIcZ-lW)   ║
# ╚══════════════════════════════════════════════════════════════════╝
...
# ── 드라이브 폴더 ID ────────────────────────────────────────────────
FOLDER_ROOT     = "1WgIJEjncgGZXz2SGQ_IhqjGSD15fz0lm"
FOLDER_HISTORY  = "1IpNRcYMc7medssJWgLhOkXrKwKOn73H3"
FOLDER_INBOX    = "1ZDyeqNXKWiJ3lQS9VcoCzeyGxsAOEsoG"
FOLDER_RESULT   = "1vXkPHr2walQJZqUrZ4tZ4FkwmQIcZ-lW"
PATTERN_FILE_ID = "1c0QRvdRIeHHmHYCVLg5l0FHsD90KTE-q"  # 발주처별_패턴통계.json

# ── 페이지 설정 ─────────────────────────────────────────────────────
st.set_page_config(page_title="투찰전략 분석 시스템 v2", page_icon="📊", layout="wide")
...
.drive-badge {background:#e0e7ff;color:#3730a3;border-radius:5px;
...
def get_drive_service():
    if not GDRIVE_AVAILABLE:
        return None
    try:
        creds_dict = st.secrets.get("GOOGLE_SERVICE_ACCOUNT", None)
        if not creds_dict:
            return None
        ...
        st.warning(f"드라이브 연결 실패: {e}")
        return None

def drive_download(service, file_id):
    """드라이브에서 파일 다운로드 → bytes"""
    try:
        ...
        st.error(f"드라이브 다운로드 오류: {e}")
        return None

def drive_upload(service, content_bytes, filename, mime_type, parent_id):
    """드라이브에 파일 업로드"""
    try:
...
# ── 낙찰이력 로드 (드라이브 우선 → 로컬 폴백) ─────────────────────
@st.cache_data(ttl=3600)
def load_history_from_drive(_service):
    """드라이브 낙찰이력데이터 폴더에서 xlsx 파일 로드"""
    if _service is None:
        return load_history_local()
    try:
        files = drive_list_files(_service, FOLDER_HISTORY)
        xlsx_files = [f for f in files if f["name"].endswith((".xlsx",".xls")) 
                      and "패턴통계" not in f["name"]]
        if not xlsx_files:
            return load_history_local()
        # 가장 최근 파일 사용
        latest = xlsx_files[0]
        content = drive_download(_service, latest["id"])
        if content:
            df = pd.read_excel(io.BytesIO(content))
            return df, latest["name"], "drive"
        return load_history_local()
    except Exception as e:
        return load_history_local()
...
def load_history_local():
    """로컬 파일 폴백"""
    local_path = "data/history.pkl"
    if os.path.exists(local_path):
        df = pd.read_pickle(local_path)
        return df, "로컬 캐시", "local"
    return None, None, None

@st.cache_data(ttl=3600)
def load_pattern_stats(_service):
    """발주처별_패턴통계.json 로드"""
    if _service is None:
        return {}
    try:
        content = drive_download(_service, PATTERN_FILE_ID)
        if content:
...
def analyze_pattern(org, df_c, pattern_stats):
    """① 발주처 패턴 분석 (패턴통계.json 우선, 없으면 직접 계산)"""
    # 패턴통계.json에서 먼저 찾기
    st_data = pattern_stats.get(org)
    if st_data:
        return {
            "pred":       st_data.get("pred_pattern", st_data.get("pred", 0)),
            "trend":      st_data.get("trend", "→횡보"),
            "pattern":    st_data.get("pattern", "무작위패턴").replace("패턴",""),
            "autocorr":   st_data.get("autocorr", 0),
            "last_val":   st_data.get("last_val", 0),
            "r5":         st_data.get("r5", 0),
            "r10":        st_data.get("r10", 0),
            "mean":       st_data.get("mean", 0),
            "std":        st_data.get("std", 0.5),
            "n":          st_data.get("n", 0),
            "recent10":   st_data.get("recent10", []),
            "source":     "패턴통계.json"
        }
    # 직접 계산
    sub = df_c[df_c["발주기관"] == org]["예가/기초(0%)"].values if df_c is not None else []
    if len(sub) < 5:
        return None
    n = len(sub)
...
def analyze_trend(org, df_c):
    ...
    return {
        "pred": round(rm + drift*0.3, 4),
        "recent_mean": round(rm, 4), "drift": round(drift, 4),
        "recent_n": rn, "recent3_mean": round(float(np.mean(r3)), 4),
        "avg_companies": round(float(co), 1) if co else None
    }
...
def parse_xls(file_bytes):
    ...
    for r in range(2, ws.nrows):
        ...
        bids.append({
            "no": int(row["번호"]), "name": row.get("공고명",""),
...
def make_excel(results):
    ...
    t.value=f"투찰전략 분석표 — {today}  ★ 3가지 분석값 (구글드라이브 연동)"
    ...
    for i,row in enumerate(results):
        ...
        if base>0:
            ...
            for ci,a,cbg in [(4,a1,BLUE),(5,a2,GREEN),(6,a3,AMBER)]:
                amt=int(base*(100+a["pred"])/100) if a else None
                if amt:
...
<h2>📊 투찰전략 분석 시스템 <span style="font-size:0.7em;opacity:0.8">v2.0</span></h2>
<p style="margin:0;opacity:0.8">3가지 분석 기반 투찰전략 자동 산출 | 구글 드라이브 연동</p>
...
# 낙찰이력 로드
df_raw, hist_name, hist_source = load_history_from_drive(service)
df_c = None
if df_raw is not None:
    df_c = df_raw[df_raw["예가/기초(0%)"].notna() & (df_raw["예가/기초(0%)"].abs() < 10)].copy()

# 패턴통계 로드
pattern_stats = load_pattern_stats(service) if drive_ok else {}
...
with st.sidebar:
    ...
    # 드라이브 연결 상태
    if drive_ok:
        st.markdown('<span class="drive-badge">🔗 구글 드라이브 연결됨</span>', unsafe_allow_html=True)
    else:
        st.warning("⚠️ 드라이브 미연결\n로컬 모드로 동작")
    ...
    if df_c is not None:
        ...
        st.caption(f"📂 출처: {hist_name}")
        if len(pattern_stats) > 0:
            st.success(f"✅ 패턴통계 로드\n{len(pattern_stats)}개 발주처")
    else:
        st.error("❌ 낙찰이력 없음")

    st.divider()
    st.caption("📁 입찰AI분석 폴더 연동")
...
if mode == "🔧 배포자 관리":
    ...
    tab1, tab2 = st.tabs(["📂 낙찰이력 업로드", "📊 드라이브 현황"])

    with tab1:
        st.subheader("낙찰이력 xlsx 업로드")
        st.info("업로드한 파일은 구글 드라이브 📁낙찰이력데이터 폴더에 자동 저장됩니다.")
        uploaded = st.file_uploader("낙찰이력 xlsx", type=["xlsx","xls"])
            ...
            with st.spinner("처리 중..."):
                try:
                    ...
                    if missing:
                        ...
                        # 드라이브 업로드
                        if drive_ok:
                            fname = f"낙찰이력_{datetime.now().strftime('%Y%m%d')}.xlsx"
                            fid = drive_upload(service, content, fname,
                                "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                FOLDER_HISTORY)
                            if fid:
                                st.success(f"✅ 드라이브 저장 완료!\n파일명: {fname}")
                        # 로컬 캐시
                        os.makedirs("data", exist_ok=True)
                        df_new.to_pickle("data/history.pkl")
                        st.cache_data.clear()
    ...
    with tab2:
        st.subheader("구글 드라이브 현황")
        if drive_ok:
            col1, col2 = st.columns(2)
            with col1:
                st.markdown("**📁 낙찰이력데이터 폴더**")
                files = drive_list_files(service, FOLDER_HISTORY)
                if files:
                    for f in files:
                        size = int(f.get("size",0))//1024
                        st.caption(f"📄 {f['name']} ({size}KB)")
                else:
                    st.caption("파일 없음")
            with col2:
                st.markdown("**📊 투찰전략결과 폴더**")
                files2 = drive_list_files(service, FOLDER_RESULT)
                if files2:
                    for f in files2[:5]:
                        st.caption(f"📄 {f['name']}")
                else:
...
# ══ 투찰전략 분석 ════════════════════════════════════════════════════
    ...
    if not xls_file:
        st.info("👆 입찰서류함 xls 파일을 업로드하면 자동으로 3가지 분석을 실행합니다.")
        st.stop()
    ...
    fname_xlsx = f"투찰전략_{today_str}.xlsx"
    ...
    with col2:
        if drive_ok:
            if st.button("☁️ 드라이브 저장 (📊투찰전략결과)", use_container_width=True):
                with st.spinner("드라이브에 저장 중..."):
                    excel_buf.seek(0)
                    fid = drive_upload(service, excel_buf.read(), fname_xlsx,
                        "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                        FOLDER_RESULT)
                    if fid:
                        st.success(f"✅ 드라이브 저장 완료!\n📊투찰전략결과/{fname_xlsx}")
                    ...
                    json_data = json.dumps([{
                        ...
                        "a1":r["a1"]["pred"] if r["a1"] else None,
                        "a2":r["a2"]["pred"] if r["a2"] else None,
                        "a3":r["a3"]["pred"] if r["a3"] else None,
                        "range_lo":r["range_lo"],"range_hi":r["range_hi"]
                    } for r in results], ensure_ascii=False, indent=2)
                    drive_upload(service, json_data.encode("utf-8"),
                        f"입찰서류함_{today_str}.json", "application/json", FOLDER_INBOX)
        else:
            st.info("드라이브 연결 시 자동 저장 가능")
